...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        if (professionalResult.rows.length > 0) {
            ...
            // CORREÇÃO: Usando a coluna correta em minúsculas
...
app.post('/api/professionals', async (req, res) => {
    ...
    try {
        ...
        // CORREÇÃO: Nomes de coluna em minúsculas para inserção
...
app.get('/api/admin/professionals', authenticateAdmin, async (req, res) => {
    ...
    try {
        client = await pool.connect();
        // CORREÇÃO FINAL: Seleciona colunas minúsculas e as renomeia (alias) para o camelCase que o frontend espera.
        const query = `
            SELECT 
                id, 
                ...
                email, 
                profession, 
                registrationnumber AS "professionalLicense", 
                registrationstatus AS "registrationStatus" 
            FROM professionals 
            ORDER BY registrationstatus ASC, id DESC`;
...
app.put('/api/admin/professionals/:id', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        // CORREÇÃO: Usa o nome da coluna em minúsculas
        ...
        // O frontend não usa a resposta aqui, mas é bom manter a consistência.
...
// Rota de pacientes (não utilizada pelo painel admin, mas mantida)
